...
#### ★ボタン→テキストリンク変換の最重要ルール★
- **ボタンを単に「→ リンクテキスト」に置き換えるのは禁止**です
- ★★★ **ボタンがあった箇所は、前後の文脈を踏まえた自然な誘導文に書き換えてください** ★★★
- ボタンの前にある段落の文末に、リンクを自然に組み込むのがベストです
- マイクロコピーやボタンテキストは「誘導の意図」を読み取り、自然な文章にリライトしてください
...
❌ NG（機械的な置き換え）:
```html
<p>→ <a href="...">SWELLを詳しく見る</a></p>
```
✅ OK（自然な文章）:
```html
<p>気になる方は<a href="..." target="_blank" rel="noopener noreferrer">SWELLの公式サイト</a>をチェックしてみてください。</p>
```
...
❌ NG:
```html
<p>→ <a href="...">無料で試してみる</a></p>
```
✅ OK:
```html
<p>無料で試せるので、まずは<a href="..." target="_blank" rel="noopener noreferrer">公式サイト</a>から触ってみるのがおすすめです。</p>
```

- テキストリンクの書き方には**2パターン**あります:

**パターン1: 文中リンク（★こちらを優先★）**
前後の文脈に自然に溶け込ませる。ボタンの変換は**必ずこのパターン**を使ってください。
```html
<!-- wp:paragraph -->
